...
import { searchScreens, isConfigured, type Platform } from "@/lib/refero";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await searchScreens({
      query,
    ...
    console.error("Refero search_screens failed:", error);
    return NextResponse.json({ error: "Failed to search screens" }, { status: 502 });
  }
}
